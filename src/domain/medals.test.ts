...
const zero: MedalCounters = {
  wordsSolid: 0, streakBest: 0, grammarDone: 0, blitzBest: 0, typedBest: 0, buildingLevels: 1, listenCorrect: 0, freezesUsed: 0, fragments: 0, errands: 0, friends: 0,
};
...
describe('линии медалей', () => {
  it('12 линий по шесть растущих порогов, включены десять', () => {
    expect(LINES).toHaveLength(12);
    for (const l of LINES) for (let i = 1; i < 6; i++) expect(l.thresholds[i]).toBeGreaterThan(l.thresholds[i - 1]);
    expect(ACTIVE_LINES.map((l) => l.id)).toEqual(['words', 'streak', 'grammar', 'cartographer', 'friend', 'courier', 'blitz', 'typed', 'builder', 'listener']);
  });
